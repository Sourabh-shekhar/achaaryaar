...
// // COMBO PRODUCT
// if (product.isCombo) {
//   const comboSize = product.comboSize || 2;
...
//     return {
//       id,
//       weights: product.comboVariants.map((variant: any) => ({
//         size: `${variant.unitWeight} × ${comboSize} jars`,
...
//   // Stock is still loading for this product.
//   if (!variants) {
//     return false;
//   }
...
//   // Variant does not exist anymore.
//   if (!variant) {
//     return true;
//   }

//   return Number(variant.stock) < item.quantity;
// }
...
export default function CartPage() {
  ...
  const productIdsKey = cartProductIds.join(",");
            ...
            try {
              ...
              if (product.isCombo) {
                ...
                if (
                  ...
                  const variants: StockInfo[] =
                    product.comboVariants.map(
                      (variant: {
                        unitWeight?: string;
                        stock?: number;
                      }) => ({
  ...
  function isOutOfStock(item: {
    ...
    const normalizedSelectedVariant =
      normalizeVariant(item.selectedVariant);

    // Find exact/normalized matching variant.
    const variant = variants.find(
      (variant) =>
        normalizeVariant(variant.size) ===
        normalizedSelectedVariant
    );

    // ------------------------------------------
    // FALLBACK FOR SINGLE OLD COMBO
    // ------------------------------------------
    // If an old combo product has only one stock
    // configuration, use that stock even if its
    // display label differs from the cart label.
    // This prevents false "Out of Stock" errors.
    // ------------------------------------------
    ...
    if (!variant && variants.length === 1) {
      ...
      if (isComboStyle) {
        return (
          onlyVariant.stock < item.quantity
        );
      }
    }
  ...
  const anyOutOfStock = items.some(
    isOutOfStock
  );
      ...
      <div className="max-w-5xl mx-auto">
                  ...
                  <div>
                    ...
                    {outOfStock && (
                      <p className="text-sm text-[#6B1F1F] font-medium mt-1">
                        This item is no longer
                        available in the quantity
                        you selected. Please remove
                        it or reduce the quantity.
                      </p>
                    )}
                  </div>
        ...
        <div className="mt-10 bg-[#3D5640] rounded-2xl shadow-2xl p-8 text-white">
          ...
          <div className="space-y-5 text-lg text-white">
            ...
            {anyOutOfStock && (
              <p className="text-center text-sm font-semibold text-[#D9A85F] bg-white/10 rounded-xl p-3">
                Please remove or adjust
                out-of-stock items before
                checking out.
              </p>
            )}
